...
export function ensureTrailingUserMessage(msgs: ModelMessage[]): ModelMessage[] {
  ...
  // observed failing request. @ai-sdk/openai-compatible's chat converter instead
  // calls `content.map()` unconditionally, so the same string throws
  // "content.map is not a function" there.
  //
